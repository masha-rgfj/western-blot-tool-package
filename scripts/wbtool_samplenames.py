...
#----------- View with the ready fugure
class FigureLabelView(QGraphicsView):
    def __init__(self, parent = None):
        super().__init__(parent)
        self.mode = None
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self.mode == "sample_label" and self.label_callback:
            scene_pt = self.mapToScene(event.pos())
            self.label_callback(scene_pt)
            self.mode = None
            return
        super().mousePressEvent(event)
...
#----------- Sample labels as independent object
class SampleLabelItem(QGraphicsTextItem):
    """Independent, movable label in the figure scene."""
    ...
    '''
    def set_width_scale(self, new_scale: float):
        self.x_scale = max(0.4, min(3.0, float(new_scale)))
        t = QTransform()
        t.scale(self.x_scale, 1.0)
        self.setTransform(t, combine=False)
    '''

    def mouseDoubleClickEvent(self, event):
        # in-place edit on double click
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        add_sample_action = QAction("Add Sample Names (click)…", self)
        add_sample_action.triggered.connect(self.enable_sample_label_mode)
        fig_menu.addAction(add_sample_action)
    ...
    def enable_sample_label_mode(self):
        self.figure_view.mode = "sample_label"
        self.figure_view.label_callback = self.add_sample_label_at_click
        print('enabled_sample_label_mode')
    
    
    def _figure_top_y(self) -> float:
    # top of all crops currently in the figure (scene coords)
        if not self.figure_bands:
            return 0.0
        tops = []
        for b in self.figure_bands:
            g = b["group"]
            # group rect top is 0 in local coords; map to scene
            tops.append(g.mapToScene(QPointF(0, 0)).y())
            return float(min(tops))

    def _default_sample_label_y(self, label_height: float) -> float:
        pad = 8.0
        return self._figure_top_y() - label_height - pad

    
    
    def add_sample_label_at_click(self, scene_pt: QPointF):
        # Find which band group was clicked
        band = self._band_at_scene_point(scene_pt)
        if not band:
            print('no band')
            return
        ...
        angle, ok = QInputDialog.getDouble(
            self, "Tilt", "Tilt (degrees):", 45.0, -180.0, 180.0, 1
        )
        if not ok:
            return
    
        group = band["group"]
        scaled_w = band["pix_item"].pixmap().width()
        if scaled_w <= 0:
            return
    
        # Convert click x to group-local x
        local_pt = group.mapFromScene(scene_pt)
        x_local = float(local_pt.x())
        #print(x_local)
    
        # Clamp to crop width (so wild clicks near edges behave)
        x_local = max(0.0, min(x_local, float(scaled_w)))
        #print(x_local)
    
        # Create label
        item = SampleLabelItem(text)
        self.figure_scene.addItem(item)

        # rotation around which point??
        br = item.boundingRect()
        item.setTransformOriginPoint(0,0)
        item.setRotation(float(-angle))

        # place at clicked x, but “top of figure” y
        y = self._default_sample_label_y(br.height())
        if angle == 0:
            item.setPos(float(scene_pt.x()) - br.width() / 2.0, y)
        else:    
            item.setPos(float(scene_pt.x()), y)
        item.setFocus()
    
    def _band_at_scene_point(self, scene_pt: QPointF):
        # Return the first band whose group contains the point (in group-local coords)
        for b in self.figure_bands:
            g = b["group"]
            p_local = g.mapFromScene(scene_pt)
            if g.rect().contains(p_local):
                return b
        return None
