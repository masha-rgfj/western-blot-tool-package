...
wbtool: created to make blots annotation easier.
If you use WBTool in published research, please cite:
Maria A. Pirozhkova (Masha) (RGFJ). \*WBTool: A GUI tool for annotating and assembling Western blot figures\* (2026).
...
class BandGroup(QGraphicsRectItem):
    def __init__(self, locked_x: float | None = None, locked_y: float | None = None):
        super().__init__()
        self.locked_x = locked_x
        self.locked_y = locked_y
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange:
            pos = QPointF(value)
            x = self.locked_x if self.locked_x is not None else pos.x()
            y = self.locked_y if self.locked_y is not None else pos.y()
            return QPointF(x, y)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.figure_top_margin = 200   # tweak: 60–120 usually feels right
        self.figure_row_y = self.figure_top_margin
        self.figure_next_x = self.figure_left_margin
        self.last_band_height = None  # for vertical crops (scale by height)
        ...
        move_down.setShortcut(Qt.Key_Left)      # ←
        move_down.triggered.connect(lambda: self.nudge_selected_x(-5))
        self.addAction(move_left)

        move_right = QAction("Move Right", self)
        move_down.setShortcut(Qt.Key_Right)      # →
        move_down.triggered.connect(lambda: self.nudge_selected_x(5))
        self.addAction(move_right)
    ...
    def open_image(self):
        ...
        try:
            start_dir = str(files("wbtool_package.data"))
        except Exception:
    ...
    def crop_region(self, scene_rect):
        ...
        self.add_vertical_crop_to_figure(cropped, inside, scene_rect, protein.strip() or "Protein")
    ...
    def add_vertical_crop_to_figure(self, pixmap: QPixmap, markers, src_scene_rect: QRectF, protein_name: str):
        """
        Optimized for VERTICAL crops (lanes): place left->right, align on same y,
        show ladder ticks only on the leftmost crop, and put protein name above rotated.
        """

    # Decide if this crop should show the ladder (only leftmost)
        show_ladder = (len(self.figure_bands) == 0)

    # Scale by HEIGHT (more natural for vertical crops)
        view_h = self.figure_view.viewport().height()
        max_h = max(50, view_h - 120)  # leave some room for label above

        target_h = int(self.last_band_height) if self.last_band_height else pixmap.height()
        target_h = min(target_h, max_h)

        scaled_pm = pixmap.scaledToHeight(target_h, Qt.SmoothTransformation)
        scale = target_h / pixmap.height()  # uniform

    # Placement: same row y, next x
        x0 = self.figure_next_x
        y0 = self.figure_row_y

    # Lock Y so everything stays aligned horizontally; allow X movement
        group = BandGroup(locked_y=y0)
        group.setRect(0, 0, scaled_pm.width(), scaled_pm.height())
        group.setPos(x0, y0)
        ...
        # Ladder ticks only for leftmost crop
        tick_items = []
        y_locals = [m["y"] - src_scene_rect.top() for m in markers]
        ...
        if show_ladder:
            ...
            for m, y_local in zip(markers, y_locals):
                ...
                line = QGraphicsLineItem(x0_tick, y, x1, y, group)
                line.setPen(pen)
                line.setAcceptedMouseButtons(Qt.NoButton)
                ...
                lab.setBrush(Qt.black)
                br = lab.boundingRect()
                lab.setPos(x0_tick - 6.0 - br.width(), y - br.height() / 2.0)
                lab.setAcceptedMouseButtons(Qt.NoButton)
    
                tick_items.append((line, lab))
    
        # Protein name ABOVE the crop, rotated (vertical)
        name_item = QGraphicsSimpleTextItem(protein_name, group)
        name_item.setFont(QFont("Arial", 12))
        name_item.setBrush(Qt.black)
        name_item.setAcceptedMouseButtons(Qt.NoButton)
    
        # Rotate around its center, then place above the crop
        br = name_item.boundingRect()
        name_item.setTransformOriginPoint(br.center())
        name_item.setRotation(-90)
    
        gap = 8
        
        # IMPORTANT: position using rotated bounds, not br
        name_item.setPos(0, 0)  # temp so mapRectToParent is stable
        rot_rect = name_item.mapRectToParent(name_item.boundingRect())

        # Move so: (1) rotated text is centered above the crop, (2) its bottom is strictly above y=0
        dx = (scaled_pm.width() / 2.0) - rot_rect.center().x()
        dy = (-gap) - rot_rect.bottom()
        name_item.moveBy(dx, dy)

        # # Now the rotated text's bottom is at y=-gap, so it never overlaps the gel.
        # # After final name_item positioning, check if it goes above y=0 (gel top)
        # name_rect = name_item.mapRectToParent(name_item.boundingRect())
        # if name_rect.top() < 0:
        #     # push the whole row down so the top becomes >= 0 (plus a little padding)
        #     pad = 4
        #     self.shift_figure_row_down(-name_rect.top() + pad)

        # # After shifting the row, the group moved down, but name_item is inside the group,
        # # so it's now safely visible without changing its local position.
        band = {
            "group": group,
            "pix_item": pix_item,
            "orig_pixmap": pixmap,
        ...
        self.figure_bands.append(band)
        self.selected_band = band
        self.last_band_height = target_h
    
        # Advance next-x so next crop appears to the right
        spacing = 30
        self.figure_next_x += scaled_pm.width() + spacing
    
        # Grow scene width if needed
        required_w = self.figure_next_x + 200
        if required_w > self.figure_scene.sceneRect().width():
            r = self.figure_scene.sceneRect()
            self.figure_scene.setSceneRect(0, 0, max(required_w, self.figure_min_width), r.height())
    
        # Ensure visible
        self.figure_view.ensureVisible(group.sceneBoundingRect().adjusted(-50, -50, 50, 50))

    def shift_figure_row_down(self, dy: float):
        """Shift all existing crops down by dy and update their locked_y."""
        if dy <= 0:
            return
    
        self.figure_row_y += dy
    
        for band in self.figure_bands:
            g = band["group"]
            # update the y lock and move item
            g.locked_y = self.figure_row_y
            g.setPos(g.pos().x(), self.figure_row_y)
    
        # grow scene height if needed
        r = self.figure_scene.sceneRect()
        needed_h = self.figure_row_y + 500  # generous buffer; you can compute tighter if you want
        if needed_h > r.height():
            self.figure_scene.setSceneRect(r.x(), r.y(), r.width(), needed_h)
    ...
    def nudge_selected_x(self, dx: int) :
        if not self.selected_band:
            return
        g = self.selected_band["group"]
        g.setPos(g.pos().x()+dx, g.pos().y())        
